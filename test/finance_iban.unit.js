...
describe('finance_iban.js', function () {
  describe('issue_944 IBAN Georgia', function () {
    ...
    it('IBAN for Georgia is correct', function () {
      ...
      assert.ok(
        iban.substring(0, 2).match(/^[A-Z]{2}$/),
        iban.substring(0, 2) +
          ' must contains only characters in GE IBAN ' +
          ibanFormated
      );
      assert.ok(
        iban.substring(2, 4).match(/^\d{2}$/),
        iban.substring(2, 4) +
          ' must contains only digit in GE IBAN ' +
          ibanFormated
      );
      assert.ok(
        iban.substring(4, 6).match(/^[A-Z]{2}$/),
        iban.substring(4, 6) +
          ' must contains only characters in GE IBAN ' +
          ibanFormated
      );
      assert.ok(
        iban.substring(6, 24).match(/^\d{16}$/),
        iban.substring(6, 24) +
          ' must contains only characters in GE IBAN ' +
          ibanFormated
      );
  ...
  describe('issue_945 IBAN Pakistan', function () {
    ...
    it('IBAN for Pakistan is correct', function () {
      ...
      assert.ok(
        iban.substring(4, 8).match(/^[A-Z]{4}$/),
        iban.substring(4, 8) +
          ' must contains only characters in PK IBAN ' +
          ibanFormated
      );
      assert.ok(
        iban.substring(8, 24).match(/^\d{16}$/),
        iban.substring(8, 24) +
  ...
  describe('issue_946 IBAN Turkish', function () {
    ...
    it('IBAN for Turkish is correct', function () {
      ...
      assert.ok(
        iban.substring(10, 26).match(/^\d{16}$/),
        'Account Code:' +
      ...
      assert.ok(
        iban.substring(2, 26).match(/^\d{24}$/),
  ...
  describe('issue_846 IBAN Azerbaijan', function () {
    ...
    it('IBAN for Azerbaijan is correct', function () {
      ...
      assert.ok(
        iban.substring(8, 28).match(/^\d{20}$/),
        iban.substring(8, 28) +
